...
evaluateString str = 
  let filtered_str = filter isNumber str
  in  read ((head filtered_str) : (last filtered_str) : []) :: Int



addAll [] = 0
addAll (first : strings) = evaluateString (first) + addAll (strings)


main = do
  putStrLn ""
  print (sum (map evaluateString input))
